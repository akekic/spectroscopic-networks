...
f2 = strcat(f1(1:k-1),'-nx-labels.lut');
...
try
	% names = dlmread(f2,'\t',1,0); % 1: skip header
	% names(:,1) = names(:,1) + 1;
	[nodes, names_temp] = textread(f2, '%u %s', 'delimiter', '\t', 'headerlines', 1);
	names = names_temp';
	% C = textscan(f2,'%u %s','Delimiter','\t');
	% celldisp(C)
	% print names
catch
...
label = names(perm);
% glabs = groups(perm,2);
glabs_temp = groups(perm);
glabs = cell2mat(glabs_temp);
[u,X] = sort(perm);
...
for i=1:m
	% changed
    hleaf(i)=plot(x(i),y(i),mar{1},'MarkerSize',msize,'MarkerFaceColor', ...
        mfc(1,:), 'MarkerEdgeColor',[0 0 0]); hold on;
